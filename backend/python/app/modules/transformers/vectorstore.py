...
def _build_text_documents(
    ...
    for block in text_blocks:
        ...
        # Token-aware (patch 16). The character cap stays as a cheap upper guard
        # for pathological blocks; the ceiling that actually matters is the
        # embedder's, in tokens, because that is the limit that rejects input.
        if (
            ...
            _ceiling = _embed_token_ceiling()
            documents.extend(
                Document(page_content=piece, metadata={**metadata, "isBlock": False})
                for chunk in _chunk_oversized_text(block_text, language)
                # A single sentence longer than the window is emitted whole by
                # _chunk_oversized_text; hard-split it rather than let it clip.
                for piece in _split_to_token_ceiling(chunk, _ceiling)
            )
            continue
        ...
        if _word_count(block_text) > _min_words_for_sentence_embeddings():
            ...
            if len(sentences) > 1:
                ...
                documents.extend(
                    Document(page_content=piece, metadata={**metadata, "isBlock": False})
                    for sentence in sentences
                    for piece in _split_to_token_ceiling(sentence, _ceiling)
                )
        documents.append(
            Document(
